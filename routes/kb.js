...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    let sql = `SELECT k.*, u.name AS author_name FROM kb_articles k
               LEFT JOIN users u ON u.id=k.author_id WHERE k.published=true`;
    const params = [];
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    ...
    const { rows: [article] } = await pool.query(
      `SELECT k.*, u.name AS author_name FROM kb_articles k
       LEFT JOIN users u ON u.id=k.author_id WHERE k.id=$1`, [req.params.id]);
    if (!article) return res.status(404).json({ error: 'Not found' });
    res.json({ success: true, data: article });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/', authenticate, authorize('admin','supervisor','agent'), async (req, res) => {
  try {
    const { title, content, category, published=false } = req.body;
    if (!title||!content) return res.status(400).json({ error: 'Title and content required' });
    const { rows: [article] } = await pool.query(
      `INSERT INTO kb_articles (title,content,category,author_id,published)
...
router.put('/:id', authenticate, authorize('admin','supervisor','agent'), async (req, res) => {
  try {
    const { title, content, category, published } = req.body;
    const { rows: [article] } = await pool.query(
      `UPDATE kb_articles SET title=COALESCE($1,title), content=COALESCE($2,content),
       category=COALESCE($3,category), published=COALESCE($4,published), updated_at=NOW()
