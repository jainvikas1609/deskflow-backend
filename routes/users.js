...
router.get('/', authenticate, async (req, res) => {
  const { rows } = await pool.query(`
    SELECT u.id, u.name, u.email, u.role, u.active, u.created_at,
           u.department, u.phone, u.company_name,
           c.name AS customer_org_name
    FROM users u
    LEFT JOIN customers c ON c.id = u.customer_org_id
    ORDER BY u.name`);
...
router.get('/agents', authenticate, async (req, res) => {
  const { rows } = await pool.query(
    `SELECT id, name, email, role FROM users
     WHERE role IN ('agent','supervisor','admin') AND active=true ORDER BY name`);
  res.json({ success: true, data: rows });
});
...
router.post('/', authenticate, authorize('admin'), async (req, res) => {
  try {
    ...
    const { rows: [user] } = await pool.query(
      `INSERT INTO users (name,email,role,password_hash,active,department,phone,company_name,customer_org_id)
       VALUES ($1,$2,$3,$4,true,$5,$6,$7,$8)
       RETURNING id,name,email,role,active,department,phone,company_name`,
  ...
  try {
    ...
    const { rows: [user] } = await pool.query(
      `UPDATE users SET ${updates.join(',')} WHERE id=$${params.length}
       RETURNING id,name,email,role,active,department,phone,company_name`, params);
