  ...
  try {
    const { rows } = await pool.query(
      `SELECT c.*, COUNT(t.id) AS ticket_count
       FROM customers c LEFT JOIN tickets t ON t.customer_org_id=c.id
...
// Create — admin and supervisor only
router.post('/', authenticate, authorize('admin','supervisor'), async (req, res) => {
  try {
    const { name, contact_email, contact_phone, account_manager, sla_tier, address, industry, website } = req.body;
    if (!name) return res.status(400).json({ error: 'Name required' });
    const { rows: [c] } = await pool.query(
      `INSERT INTO customers (name,contact_email,contact_phone,account_manager,sla_tier,address,industry,website)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [name,contact_email,contact_phone,account_manager,sla_tier||'Standard',address,industry||null,website||null]);
    res.status(201).json({ success: true, data: c });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Edit — admin only (not supervisor, not agent)
router.put('/:id', authenticate, authorize('admin'), async (req, res) => {
  try {
    const fields = ['name','contact_email','contact_phone','account_manager','sla_tier','address','industry','website','active'];
